...
ModLuaFileAppend(
    ...
    "mods/quant.ew/files/system/perk_patches/append/cosmetics_append.lua"
)
...
if not ctx.is_host or not ctx.proxy_opt.randomize_perks then
    ...
    ModLuaFileAppend(
        "data/scripts/perks/perk_list.lua",
        "mods/quant.ew/files/system/perk_patches/append/perks_no_telekinesis.lua"
    )
end
...
local function become_fungus(entity_who_picked)
    ...
    if damagemodels ~= nil then
        for i, damagemodel in ipairs(damagemodels) do
            local explosion_resistance =
                tonumber(ComponentObjectGetValue(damagemodel, "damage_multipliers", "explosion"))
