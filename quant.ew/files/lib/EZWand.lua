...
local function render_tooltip(origin_x, origin_y, wand, gui_, z)
    ...
    gui_text_with_shadow(
        gui,
        0,
        5,
        GameTextGetTranslatedOrNot(wand.props.shuffle and "$menu_yes" or "$menu_no"),
        1,
        z - 1
    )
    ...
    if type(wand.always_cast_spells) == "table" and #wand.always_cast_spells > 0 then
        ...
        for i, spell in ipairs(wand.always_cast_spells) do
            ...
            GuiImage(
                ...
                x + 2,
                y + 2,
                (spell_lookup[spell] and spell_lookup[spell].icon) or "data/ui_gfx/gun_actions/unidentified.png",
                1,
                always_cast_spell_icon_scale,
                always_cast_spell_icon_scale
            )
        end
        GuiLayoutEnd(gui)
    ...
    for i = 1, wand.props.capacity do
        ...
        GuiImage(
            ...
            -0.3,
            -0.4,
            "data/ui_gfx/inventory/inventory_box.png",
            0.95,
            background_scale,
            background_scale
        )
        ...
        if not wand.spells[i] or wand.spells[i] == "" then
            ...
            GuiImage(
                ...
                x + 0.11,
                y,
                (spell_lookup[wand.spells[i]] and spell_lookup[wand.spells[i]].icon)
                    or "data/ui_gfx/gun_actions/unidentified.png",
                0.8,
                spell_icon_scale,
                spell_icon_scale
            )
...
function wand:new(from, rng_seed_x, rng_seed_y)
    ...
    elseif tonumber(from) or type(from) == "number" then
        ...
        if starts_with(from, "EZW") then
            ...
            protected.entity_id = EntityLoad("data/entities/items/wand_level_04.xml", rng_seed_x or 0, rng_seed_y or 0)
            protected.ability_component =
                EntityGetFirstComponentIncludingDisabled(protected.entity_id, "AbilityComponent")
            protected.item_component = EntityGetFirstComponentIncludingDisabled(protected.entity_id, "ItemComponent")
...
function wand:_AddSpells(spells, attach)
    ...
    if not attach and #spells_on_wand + count > self.capacity then
        error(
            string.format("Wand capacity (%d/%d) cannot fit %d more spells. ", #spells_on_wand, self.capacity, count),
            3
        )
    end
    ...
    while i <= #spell_args do
        if type(spell_args[i]) == "table" then
            ...
            if type(spell_args[i][1]) ~= "string" or type(spell_args[i][2]) ~= "number" then
                error(
                    "Wrong argument format at index "
                        .. i
                        .. '. Expected format for multiple spells shortcut: { "BOMB", 3 }',
                    3
                )
            else
    ...
    for _, spell in ipairs(children) do
        ...
        if action_id then
            if permanent == true then
                table.insert(
                    always_cast_spells,
                    { action_id = action_id, entity_id = spell, inventory_x = inventory_x, inventory_y = inventory_y }
                )
            else
                table.insert(
                    spells,
...
function wand:_RemoveSpells(spells_to_remove, detach)
    ...
    for i, v in ipairs(which) do
        if
            #spells_to_remove == 0
            or spells_to_remove_remaining[v.action_id] and spells_to_remove_remaining[v.action_id] ~= 0
        then
            if #spells_to_remove > 0 then
...
function wand:PickUp(entity)
    ...
    local sprite_particle_emitter_comp =
        EntityGetFirstComponentIncludingDisabled(self.entity_id, "SpriteParticleEmitterComponent")
        ...
        set = function(self, value)
            ...
            if self.hotspot_comp then
                ComponentSetValue2(
                    self.hotspot_comp,
                    "offset",
                    value and self.shoot_offset_x or 0,
                    value and self.shoot_offset_y or 0
                )
            end
...
local function create_virtual_wand(from, rng_seed_x, rng_seed_y)
    ...
    local holder_entity, inv_quick, controls_comp, character_platforming_component, platform_shooter_player_comp =
        create_holder_entity()
...
function wand:Deploy(x, y)
    ...
    if EntityGetName(root) ~= "EZWand_virtual_wand" then
        ...
        protected.hotspot_comp =
            EntityGetFirstComponentIncludingDisabled(wand_entity_id, "HotspotComponent", "shoot_pos")
        if protected.hotspot_comp then
            protected.shoot_offset_x, protected.shoot_offset_y = ComponentGetValue2(protected.hotspot_comp, "offset")
        end
        ...
        protected.character_platforming_component =
            EntityGetFirstComponentIncludingDisabled(holder_entity, "CharacterPlatformingComponent")
        protected.platform_shooter_player_comp =
            EntityGetFirstComponentIncludingDisabled(holder_entity, "PlatformShooterPlayerComponent")
        return o
    end
end
