...
add_dylib_debug: extract_steam_redist
    mkdir target/debug/ -p
    cp redist/libsteam_api.so target/debug/

add_dylib_release: extract_steam_redist
    mkdir target/release/ -p
    cp redist/libsteam_api.so target/release/
...
build_noita_api_example:
    ...
    cp target/i686-pc-windows-gnu/release/noita_api_example.dll noita_api_example/material_converter/material_converter.dll
...
build_ext_debug:
    ...
    cp target/i686-pc-windows-gnu/debug/ewext.dll quant.ew/ewext.dll
...
build_injector:
    ...
    cp target/i686-pc-windows-gnu/release/dll_injector.exe $HOME/.local/share/Steam/steamapps/common/Noita/noita.exe

build_malloc:
    cd extra/malloc_probe && cargo build --release --target=i686-pc-windows-gnu
    cp target/i686-pc-windows-gnu/release/malloc_probe.dll $HOME/.local/share/Steam/steamapps/common/Noita
...
build_malloc_log:
    cd extra/malloc_probe && cargo build --release --features "log" --target=i686-pc-windows-gnu
    cp target/i686-pc-windows-gnu/release/malloc_probe.dll $HOME/.local/share/Steam/steamapps/common/Noita

build_malloc_log_debug:
    cd extra/malloc_probe && cargo build  --features "log" --target=i686-pc-windows-gnu
    cp target/i686-pc-windows-gnu/debug/malloc_probe.dll $HOME/.local/share/Steam/steamapps/common/Noita
...
run-w-gdb: add_dylib_debug build_ext
    cd noita_proxy && NP_SKIP_MOD_CHECK=1 cargo run -- --run-noita-with-gdb --launch-cmd "wine noita.exe"
