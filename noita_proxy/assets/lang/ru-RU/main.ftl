...
launcher_no_command_3 = Вы можете вставить `noita_proxy --launch-cmd "%command%"` в настройки запуска Steam чтобы перехватить команду, которую Steam использует для запуска.
...
local_health_alternate_dont_run = Статуя возрождения не бежит
revive_on_drop = Возрождать игрока в любом месте при сбросе сердца
